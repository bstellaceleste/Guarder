...
private:
	...
	public:
		void initialize(unsigned classSizePowerMax)
		{
			...
			for (i = minClassSizePower + 1; i <= numClassSizePowers; i++)
			{
				...
				//PRINT("Guarder------Class_size: %d", (int)pow(2, i));
			}
	...
	public:
		...
		int repopulate()
		{
			...
			if (i > 0)
			{
				PRDBG("received %d objects from global cache (numObjectsToTransfer=%u, "
					  "numGlobalFreeObjects=%u)",
					  i, numObjectsToTransfer, numGlobalFreeObjects);
			}
		...
		inline void *getNext()
		{
			...
			/*PRINT("freeCache->getNext(): retval=%p, this=%p, _numFreeObjects=%lu, _capacity=%lu, _start=0x%lx, _end=0x%lx",
...
#pragma message "allocation buffer refill level set to 75%"
		...
		{ 
			...
			// PRINT("trying object %p at index %d...", object, index);
			...
			{
				//PRINT("freecache has free objects, doing replacement of cache index %u", index);
...
#pragma message "BIBOP_BAG_SIZE in use: overrides randomized bag size"
		...
		_bibopBagSize = MIN_RANDOM_BAG_SIZE << randPower; bagsize=_bibopBagSize;
#endif
...
//SPP c ici qu'il faudrait définir la freq comme dans Slimguard
				...
				// PRDBG("thread %d bag %d: heap start @ 0x%lx, shadow mem @ 0x%lx, free cache @ 0x%lx, Cache=%p, FreeCache=%p", threadNum, bagNum, bagStart, bagShadowMemStart, bagFreeAreaStart, curBag->cache, curBag->freeCache);

#ifdef ENABLE_GUARDPAGE
				if( (classSize % PAGESIZE) == 0 )
				{//PRINT("Guarder------- ENABLE_GUARDPAGE__1- size %d", classSize);
...
#ifdef ENABLE_GUARDPAGE
				if( (classSize % PAGESIZE) == 0 )
				{//PRINT("Guarder------- ENABLE_GUARDPAGE__2- size %d", classSize);
					// We must perform this >1 test or else we will underflow this value
					// for bags which have 0 objects in them. Also, the last usable bag
					// will only contain a single object, but it will not be reduced to
					// make room for a guard page, but rather the next bag (which is
...
#ifdef ENABLE_GUARNARY //SPP -- if( !(classSize%PAGESIZE) ) alors GP sinon SPP
				//PRINT("Guarder------- ENABLE_GUARNARY");
				if( (classSize % PAGESIZE) != 0 )
				{
					get_pattern(curBag);
        			madvise((void *)bagStart, _bibopBagSize, 20 + curBag->pattern);
				}
#endif
			}
	...
	void *allocateSmallObject(size_t sz)
	{
		...
		// PRINF("allocateSmallObject: classSizePower %d ", classSizePower);
	...
	void freeSmallObject(void *addr)
	{
		...
		//PRINT("Guarder------- Thread %d - Bag %d - Size %d", bag->bagNum, bag->threadIndex, bag->classSize);
